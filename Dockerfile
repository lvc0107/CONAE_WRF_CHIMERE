...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && cd openmpi-2.0.4 \
 && ./configure CC=/usr/bin/gcc CXX=/usr/bin/g++ FC=/usr/bin/gfortran CFLAGS=-m64 CXXFLAGS=-m64 FCFLAGS=-m64 --prefix=/usr/local &> /chim/libs/build_log_openmpi_config \
 && make all install &> /chim/libs/build_log_openmpi_make \
 && cd / \
 && rm -rf /chim/libs/openmpi/BUILD_DIR
...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && cd hdf5-1.8.20 \
 && make clean \
 && ./configure CC=/usr/local/bin/mpicc FC=/usr/local/bin/mpif90 CPPFLAGS=-I/usr/local/include/openmpi LDFLAGS=-L/usr/local/lib/openmpi/ --enable-fortran --enable-parallel --prefix=/usr/local/ &> /chim/libs/build_log_hdf5_config \
 && make install &> /chim/libs/build_log_hdf5_make \
 && rm -rf /chim/libs/hdf5/BUILD_DIR
...
RUN source /opt/rh/devtoolset-8/enable \
 ...
 && cd netcdf-c-4.6.0 \
 && ./configure CC=/usr/local/bin/mpicc CPPFLAGS="-I/usr/local/include/openmpi -I/usr/local/include" LDFLAGS="-L/usr/local/lib/openmpi/ -L/usr/local/lib" --prefix=${NETCDF} &> /chim/libs/build_log_ncc_config \
 && make install &> /chim/libs/build_log_ncc_make
RUN source /opt/rh/devtoolset-8/enable \
 && env \
 && cd ${NETCDF}/BUILD_DIR \
